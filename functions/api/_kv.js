...
// 依赖 KV 绑定；用于评论/留言/看板防刷。
export async function rateLimit(env, key, max, windowSec) {
  const now = Date.now();
  let data = null;
  try { data = await env.NEWS_KV.get(key, { type: "json" }); } catch (e) { data = null; }
  ...
  } catch (e) { /* 限流写入失败不阻断主流程 */ }
  return true;
}

// 从请求头取访客真实 IP（只信 Cloudflare 边缘注入的 CF-Connecting-IP，
// 不信任 x-forwarded-for / x-real-ip —— 这两个可被客户端伪造，绕过按 IP 限流）
